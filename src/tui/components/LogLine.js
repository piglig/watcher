/**
 * LogLine — unified log entry renderer used by every run screen.
 *
...
export default function LogLine({ line }) {
  ...
  // Tolerate ERR/WARN bracket prefixes that ScrapeRun emits ("[ERR] xxx")
  // by classifying on the body but stripping the bracket before display.
  ...
  return (
    <Box gap={1}>
      {ts ? <Text color="gray" dimColor>{ts.padEnd(8)}</Text> : <Text color="gray" dimColor>{'        '}</Text>}
