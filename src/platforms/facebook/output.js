/**
 * facebook-output.js — Terminal table, stats, JSON and CSV for Facebook posts
 */
...
export function printFacebookTable(posts) {
  ...
  posts.forEach((p, i) => {
    ...
    const row     = [
      ...
      formatNumber(p.metrics?.likes    ?? 0).padEnd(cols.Likes),
      formatNumber(p.metrics?.comments ?? 0).padEnd(cols.Comments),
      formatNumber(p.metrics?.shares   ?? 0).padEnd(cols.Shares),
...
export function printFacebookStats(profile, posts) {
  if (!posts.length) return;

  const status    = posts.filter(p => p.type === 'status');
  ...
  const totShares   = posts.reduce((s, p) => s + (p.metrics?.shares   ?? 0), 0);
  const totViews    = posts.reduce((s, p) => s + (p.metrics?.views    ?? 0), 0);
  const top = [...posts].sort((a, b) => (b.metrics?.likes ?? 0) - (a.metrics?.likes ?? 0))[0];
  ...
  console.log(`  Total posts    : ${posts.length} (status: ${status.length}, photo: ${photos.length}, video: ${videos.length}, carousel: ${carousels.length})`);
  ...
  if (top) {
    console.log(`\n  Top post (${formatNumber(top.metrics?.likes ?? 0)} likes):`);
    console.log(`  ${top.url}`);
...
export function toFacebookJSON(profile, posts) {
...
export function toFacebookCSV(posts) {
